...
import { createTimingConfig, getContentTransform, getDrawerTransform } from '@/src/utils/drawerAnimations';
...
const SingleDrawer: React.FC<SingleDrawerProps> = ({
  ...
  // Get the drawer config - this should include all default values
  const config = drawer.config;

  useEffect(() => {
    // Opening animation
    progress.value = withTiming(1, createTimingConfig(config.transitionDuration));
    return () => {
      // Cleanup function - this runs when component unmounts
      // But we want to handle closing animation differently
    };
  ...
  const drawerStyle = useAnimatedStyle(() => {
      ...
      // Add keyboard height to bottom position when keyboard is open
...
export const Drawer: React.FC<DrawerProps> = ({ children }) => {
  ...
  useEffect(() => {
    if (topDrawer && topDrawer.config.transitionType === 'push') {
      ...
      // Use the previous topDrawer's config for closing animation if available
