...
const AttributeStep = ({
  ...
  const handleSelectAttribute = useCallback(
    (attribute: Service) => {
      if (attribute.attributes?.length && attribute.openDrawer) {
        ...
        attribute.attributes[0]?.attributes?.length
      ) {
        setSelected((prev) => ({
          ...prev,
  ...
  const handleUnselectAttribute = useCallback(
    (attribute: Service) => {
      const slugsToRemove = extractChildren(attribute, []).map((e) => e.id);
  ...
  const isAttributeSelected = useCallback(
    (attribute: Service) => {
      return Object.keys(selected.options).find(
        (e) =>
          attribute.id.toString() === e ||
          findAncestors(services, e)
            ?.map((k) => k?.id)
            ?.includes(attribute.id),
      );
    },
  ...
  const NailHeader = useMemo(() => {
    ...
    const hand = selected?.service?.attributes?.find(
      (e) => e.slug === ServiceEnum.Hand,
    );
    const feet = selected?.service?.attributes?.find(
      (e) => e.slug === ServiceEnum.Feet,
    );
        ...
        >
          <TextView
            style={[
              ...
              nailTab?.slug === feet?.slug &&
                attributeStepStyles.selectedButtonText,
            ]}
          >
            {feet?.title}
          </TextView>
  ...
  useEffect(() => {
    if (selected?.service?.slug === ServiceEnum.Nail) {
      ...
      setAttributes(
        (selected?.attributeStep || selected?.service)?.attributes || [],
      );
    }
            ...
            <Animated.View key={attribute.slug}>
              ...
              >
                ...
                <View style={attributeStepStyles.attributeCardRight}>
                  <TextView style={styles.attributeTitle}>
                    {attribute.title}
                  </TextView>
