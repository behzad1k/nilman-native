...
const SecAttrDrawer = ({
  ...
  const [currentParent, setCurrentParent] = useState<Service | undefined>(
    parent,
  );
  ...
  const validateColorSelection = useCallback(() => {
    ...
    for (const optionId of optionIds) {
      const service = services.allServices.find(
        (s) => s.id.toString() === optionId,
      );
  ...
  const handleAddAttribute = useCallback(
    (secAttr: Service | null, color?: string | null) => {
      if (!secAttr) return;
      ...
      if (
        ...
        Object.keys(selected.options).some(
          (e) =>
            services.allServices?.find((j) => e === j.id.toString())?.parent
              ?.id === activeParent.id,
        )
      ) {
        // Show error toast
  ...
  return (
    <View>
      <View style={styles.drawerContainer}>
        <View style={secAttrDrawerStyles.header}>
          <View style={secAttrDrawerStyles.headerActions}>
            ...
            <TouchableOpacity
              style={styles.headerButton}
              onPress={() =>
                openDrawer("infoDrawer", <InfoDrawer parent={activeParent} />, {
                  drawerHeight: "auto",
                })
              }
            >
              <TextView style={secAttrDrawerStyles.moreInfoText}>
                توضیحات
              </TextView>
        ...
        <View style={secAttrDrawerStyles.content}>{renderContent()}</View>
      </View>
