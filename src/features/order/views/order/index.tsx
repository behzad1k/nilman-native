...
const OrderPage = () => {
  ...
  const { isUrgent = false, isMulti = false } = route.params || {};
  ...
  const { execute: submitOrder, loading: isLoading } = useAsyncOperation();
  ...
  const totalPrice = useMemo(() => {
    return (
      selected.attributes?.reduce((total, attr) => total + attr.price, 0) || 0
    );
  }, [selected.attributes]);
    ...
    for (const [serviceId, options] of Object.entries(selected.options)) {
      ...
      if (options.addOns) {
        for (const addOnId of Object.keys(options.addOns)) {
          const addOnService = allServices.find(
            (s) => s.id.toString() === addOnId,
          );
  ...
  const saveOrderData = useCallback(async () => {
    try {
      await Promise.all([services.order.setSavedOrder(selected, step)]);
    } catch (error) {
  ...
  const handleChangeStep = useCallback(
    async (action: "next" | "prev") => {
      if (action === "next") {
        ...
        if (step?.index === 1) {
          const newParent = allServices.find(
            (e) => e.id === selected.attributeStep?.parent?.id,
          );
          if (!newParent) {
            setStep(STEPS[0]);
            setSelected((prev) => ({
            ...
            setSelected((prev) => ({
              ...prev,
              attributeStep: allServices?.find(
                (e) => e.id === prev.attributeStep?.parent?.id,
              ),
            }));
          }
  ...
  const handleSubmitOrder = useCallback(async () => {
    ...
    try {
      const res = await submitOrder(() =>
        services.order.submitOrder(selected, allServices),
      );
  ...
  return (
    <View
      style={[
        orderStyles.main,
        Platform.OS == "web" && orderStyles.paddingBottom84,
      ]}
    >
          ...
          <View style={orderStyles.buttonSection}>
            {step?.index === STEPS.length - 1 ? (
              <TouchableOpacity
                ...
                activeOpacity={0.8}
              >
                <TextView style={orderStyles.submitButtonText}>
                  ثبت سفارش
                </TextView>
              </TouchableOpacity>
            ) : (
              ...
              >
                <TextView
                  style={[
                    ...
                    !isNextStepAllowed
                      ? orderStyles.disabledButtonText
                      : undefined,
                  ]}
                >
                  ادامه
