...
export default function OrderCard({ item }: IOrderCardProps) {
  ...
  const { execute: submit, loading } = useAsyncOperation();
    ...
    <View style={styles.orderCardContainer}>
      <View style={orderCardStyles.orderCardInfo}>
        <View style={orderCardStyles.profilePicture}>
          <Image
            source={
              item?.worker?.profilePic?.url
                ? { uri: item?.worker?.profilePic?.url }
        ...
        <View style={orderCardStyles.orderCardInfoHeader}>
          <TextView style={orderCardStyles.serviceTitle}>
            {item.service.title}
          </TextView>
      ...
      <View style={orderCardStyles.orderCardSecRow}>
        ...
        <View style={orderCardStyles.orderCardService}>
          {item.orderServices
            ...
            .slice(0, 3)
            ?.map((attribute, index) => {
              const attrFirstAncestor = findAncestors(
                allServices,
                attribute.serviceId,
              ).find((e) => e?.showInList && e?.openDrawer);
              return (
                <View key={index} style={orderCardStyles.orderCardServiceIcon}>
                  <View style={orderCardStyles.serviceCountBadge}>
                    <TextView style={orderCardStyles.serviceCountText}>
      ...
      <View style={orderCardStyles.orderCardButtons}>
        <TouchableOpacity
          ...
          onPress={() =>
            openDrawer("orderDetail", <OrderDetail item={item} />, {
              drawerHeight: "auto",
            })
          }
        >
          <TextView style={orderCardStyles.viewInvoiceButtonText}>
            مشاهده فاکتور
          </TextView>
        </TouchableOpacity>
        ...
        >
          <TextView style={orderCardStyles.reOrderButtonText}>
            سفارش مجدد
          </TextView>
        </TouchableOpacity>
      </View>
